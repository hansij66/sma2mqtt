...
class Discovery(threading.Thread):
  ...
  def run(self):
    ...
    if cfg.HA_DISCOVERY:
      while not self.__stopper.is_set():
        ...
        if t_elapsed > self.__interval:
          for dictionary in self.__listofjsondicts:
            topic = "homeassistant/sensor/" + cfg.HA_MQTT_DISCOVERY_TOPIC_PREFIX + "/" + dictionary["unique_id"] + "/config"
            self.__mqtt.do_publish(topic, json.dumps(dictionary, separators=(',', ':')), retain=True)
    ...
    if cfg.HA_DELETECONFIG:
      for dictionary in self.__listofjsondicts:
        topic = "homeassistant/sensor/" + cfg.MQTT_TOPIC_PREFIX + "/" + dictionary["unique_id"] + "/config"
